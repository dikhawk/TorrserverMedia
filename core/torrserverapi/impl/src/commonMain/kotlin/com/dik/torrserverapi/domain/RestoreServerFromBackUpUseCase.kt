...
class RestoreServerFromBackUpUseCase(
    ...
    suspend operator fun invoke(pathToBackupFile: String, pathToFile: String): Result<Unit, TorrserverError> {
        Logger.Companion.i("$tag start restore server $pathToFile from backup file $pathToBackupFile")
        ...
        return withContext(dispatchers.ioDispatcher()) {
            ...
            try {
                if (!fileSystem.exists(backUpFile)) {
                    Logger.Companion.e("$tag backup file not exist $pathToBackupFile")
                ...
                Logger.Companion.i("$tag file restored from backup successfully")
